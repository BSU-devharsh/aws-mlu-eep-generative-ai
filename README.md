...
The book lives in the [`book/`](book/) directory.
...
**Publish it (GitHub Pages):** the included workflow
[`.github/workflows/deploy.yml`](.github/workflows/deploy.yml) builds and deploys
the book automatically. In the repository, set **Settings → Pages → Source** to
**GitHub Actions**, then push to `main` (or run the workflow manually). The site
will be live at `https://<your-username>.github.io/<repo>/`.

The lab notebooks are rendered for reading but **not executed** during the book
